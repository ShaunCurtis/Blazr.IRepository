...
public sealed class WeatherForecastListService
{
    ...
    public async ValueTask GetForecastsAsync(ListQueryRequest<WeatherForecast> request)
    {
    ...
    {
        var request = new ListQueryRequest<WeatherForecast>() { StartIndex = itemsRequest.StartIndex, PageSize = itemsRequest.Count };
